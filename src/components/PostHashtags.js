...
export default function PostHashtags(props){
    ...
    const [editInput, setEditInput] = useState(allHashtags.description);
    ...
    async function deletePost() {
      ...
      const URL = `https://linkr-api-0l14.onrender.com/posts/${allHashtags.id}`;
    ...
    return(<>
                ...
                {allHashtags.map((h)=>{const {id,pic,preview,title,text,userName,userId,picture,description,link}= h;
                return(
                    <PostCard>
        <div>
          <img src={picture} />
        ...
        <PostContent>
          ...
          <LinkDisplayer onClick={() => linkRedirection(link)}>
            <LinkInfo>
              <h2>{title}</h2>
              <h3>{preview}</h3>
              <h4>{link}</h4>
